...
impl AMMContract {
    pub fn storage_use(&mut self, sender_id: &AccountId, initial_storage_usage: StorageUsage, initial_user_balance: Balance) {
...
impl AMMContract {
    ...
    #[payable]
    pub fn storage_withdraw(&mut self, amount: U128) -> AccountStorageBalance {
        assert_one_yocto();
...
mod mock_token_basic_tests {
    ...
    fn storage_manager_deposit() {
        ...
        // let mut contract = AMMContract::init(gov(), vec![]);
    ...
    fn storage_manager_withdraw_too_much() {
        testing_env!(get_context(token()));
        let whitelist = Some(vec![to_valid(bob()), to_valid(carol())]);
        let mut contract = AMMContract::init(gov(), config());
